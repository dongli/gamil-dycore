...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i, k, reduced_tend)
    do j = full_lat_start_idx_no_pole_a_grid(pole), full_lat_end_idx_no_pole_a_grid(pole)
      if (full_reduce_factor(j) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%fv_a(i,j) = (coef%cori(j) + coef%curv(j) * state%u_a(i,j)) * state%iap%v_a(i,j)
        end do
      else
        tend%fv_a(:,j) = 0.0
        do k = 1, full_reduce_factor(j)
          do i = reduced_start_idx_at_full_lat(j), reduced_end_idx_at_full_lat(j)
            reduced_tend(i) = (coef%cori(j) + coef%curv(j) * full_reduced_state(j)%u_a(i,k,2)) * full_reduced_state(j)%iap%v_a(i,k,2)
          end do
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i, k, reduced_tend)
    do j = full_lat_start_idx_no_pole_a_grid(pole), full_lat_end_idx_no_pole_a_grid(pole)
      if (full_reduce_factor(j) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%fu_a(i,j) = (coef%cori(j) + coef%curv(j) * state%u_a(i,j)) * state%iap%u_a(i,j)
        end do
      else
        tend%fu_a(:,j) = 0.0
        do k = 1, full_reduce_factor(j)
          do i = reduced_start_idx_at_full_lat(j), reduced_end_idx_at_full_lat(j)
            reduced_tend(i) = (coef%cori(j) + coef%curv(j) * full_reduced_state(j)%u_a(i,k,2)) * full_reduced_state(j)%iap%u_a(i,k,2)
          end do
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(c1, c2, i, k, reduced_tend)
    do j = full_lat_start_idx_no_pole_c_grid, full_lat_end_idx_no_pole_c_grid
      ...
      if (full_reduce_factor(j) == 1) then
        do i = parallel%half_lon_start_idx, parallel%half_lon_end_idx
          tend%fv_c(i,j) = 0.25 * (coef%cori(j) + coef%curv(j) * state%u_c(i,j)) * &
        ...
        do k = 1, full_reduce_factor(j)
          do i = reduced_start_idx_at_full_lat(j), reduced_end_idx_at_full_lat(j)
            reduced_tend(i) = 0.25 * (coef%cori(j) + coef%curv(j) * full_reduced_state(j)%u_c(i,k,2)) * &
      ...
      if (full_reduce_factor(j) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%fu_c(i,j) = 0.25 * (coef%cori(j) * (state%iap%u_c(i,j) + state%iap%u_c(i-1,j)) + &
                                   coef%curv(j) * (state%u_c(i,  j) * state%iap%u_c(i,  j) + &
        ...
        do k = 1, full_reduce_factor(j)
          do i = reduced_start_idx_at_full_lat(j), reduced_end_idx_at_full_lat(j)
            reduced_tend(i) = 0.25 * &
              (coef%cori(j) * (full_reduced_state(j)%iap%u_c(i,k,2) + full_reduced_state(j)%iap%u_c(i-1,k,2)) + &
               coef%curv(j) * (full_reduced_state(j)%u_c(i,  k,2) * full_reduced_state(j)%iap%u_c(i,  k,2) + &
      ...
      if (full_reduce_factor(j+1) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%fu_c(i,j) = tend%fu_c(i,j) + 0.25 * (coef%cori(j+1) * (state%iap%u_c(i,j+1) + state%iap%u_c(i-1,j+1)) + &
                                                    coef%curv(j+1) * (state%u_c(i,  j+1) * state%iap%u_c(i,  j+1) + &
        ...
        do k = 1, full_reduce_factor(j+1)
          do i = reduced_start_idx_at_full_lat(j+1), reduced_end_idx_at_full_lat(j+1)
            reduced_tend(i) = 0.25 * &
              (coef%cori(j+1) * (full_reduced_state(j+1)%iap%u_c(i,k,2) + full_reduced_state(j+1)%iap%u_c(i-1,k,2)) + &
               coef%curv(j+1) * (full_reduced_state(j+1)%u_c(i,  k,2) * full_reduced_state(j+1)%iap%u_c(i,  k,2) + &
...
!$OMP END PARALLEL DO
  ...
  subroutine space_operators_run_on_a_grids(state, tend, dt, pass, pole)
    ...
    case (all_pass)
      ...
      do j = full_lat_start_idx_a_grid(pole), full_lat_end_idx_a_grid(pole)
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%du_a(i,j) = - tend%u_adv_lon_a(i,j) - tend%u_adv_lat_a(i,j) + tend%fv_a(i,j) - tend%u_pgf_a(i,j)
        end do
      end do

      do j = full_lat_start_idx_a_grid(pole), full_lat_end_idx_a_grid(pole)
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%dv_a(i,j) = - tend%v_adv_lon_a(i,j) - tend%v_adv_lat_a(i,j) - tend%fu_a(i,j) - tend%v_pgf_a(i,j)
        end do
      end do
    ...
    case (fast_pass)
      ...
      do j = full_lat_start_idx_a_grid(pole), full_lat_end_idx_a_grid(pole)
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%du_a(i,j) = tend%fv_a(i,j) - tend%u_pgf_a(i,j)
        end do
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SMOOTHING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if (filter_full_zonal_tend(j)) then
          s1 = sum(tend%du_a(i1:i2,j) * state%iap%u_a(i1:i2,j))
      ...
      do j = full_lat_start_idx_a_grid(pole), full_lat_end_idx_a_grid(pole)
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%dv_a(i,j) = - tend%fu_a(i,j) - tend%v_pgf_a(i,j)
        end do
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SMOOTHING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if (filter_full_zonal_tend(j)) then
          s1 = sum(tend%dv_a(i1:i2,j) * state%iap%v_a(i1:i2,j))
  ...
  subroutine fill_ac_interface(state)
    ...
    ! A grids at South Pole
    j = full_lat_end_idx_a_grid(south_pole) + 1
    do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
      state%u_a(i,j) = (state%u_c(i-1,j) + state%u_c(i,j)) * 0.5
      state%v_a(i,j) = (state%v_c(i,j-1) + state%v_c(i,j)) * 0.5
    end do
    call parallel_fill_halo(state%u_a(:,j), left_halo=.true., right_halo=.true.)
    call parallel_fill_halo(state%v_a(:,j), left_halo=.true., right_halo=.true.)
    ...
    ! A grids at North Pole
    j = full_lat_start_idx_a_grid(north_pole) - 1
    do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
      state%u_a(i,j) = (state%u_c(i-1,j) + state%u_c(i,j)) * 0.5
      state%v_a(i,j) = (state%v_c(i,j-1) + state%v_c(i,j)) * 0.5
    end do
    ...
    call parallel_fill_halo(state%iap%u_c(:,j), left_halo=.true., right_halo=.true.)
  ...
  subroutine check_antisymmetry(tend, state)
    ...
    do j = full_lat_start_idx_a_grid(south_pole), full_lat_end_idx_a_grid(south_pole)
      do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
        ip_u_adv_lon = ip_u_adv_lon + tend%u_adv_lon_a(i,j) * state%iap%u_a(i,j) * mesh%full_cos_lat(j)
        ip_u_adv_lat = ip_u_adv_lat + tend%u_adv_lat_a(i,j) * state%iap%u_a(i,j) * mesh%full_cos_lat(j)
        ip_fv = ip_fv + tend%fv_a(i,j) * state%iap%u_a(i,j) * mesh%full_cos_lat(j)
        ip_u_pgf = ip_u_pgf + tend%u_pgf_a(i,j) * state%iap%u_a(i,j) * mesh%full_cos_lat(j)
        ip_v_adv_lon = ip_v_adv_lon + tend%v_adv_lon_a(i,j) * state%iap%v_a(i,j) * mesh%full_cos_lat(j)
        ip_v_adv_lat = ip_v_adv_lat + tend%v_adv_lat_a(i,j) * state%iap%v_a(i,j) * mesh%full_cos_lat(j)
        ip_fu = ip_fu + tend%fu_a(i,j) * state%iap%v_a(i,j) * mesh%full_cos_lat(j)
        ip_v_pgf = ip_v_pgf + tend%v_pgf_a(i,j) * state%iap%v_a(i,j) * mesh%full_cos_lat(j)
      end do
    end do
