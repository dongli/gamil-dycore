...
contains
  ...
  subroutine restart_init()
    ...
    call io_add_var('u_c', long_name='u_c wind component', units='m s-1', dim_names=['ilon', 'lat ', 'time'], dataset_name='restart')
    call io_add_var('v_c', long_name='v_c wind component', units='m s-1', dim_names=['lon ', 'ilat', 'time'], dataset_name='restart')
  ...
  subroutine restart_read(state, static)
    ...
    call io_input('u_c', state%u_c, 'restart')
    call parallel_fill_halo(state%u_c(:,:), all_halo=.true.)
    call io_input('v_c', state%v_c, 'restart')
  ...
  subroutine restart_write(state, static)
    ...
    call io_output('u_c', state%u_c(:,:), 'restart')
    call io_output('v_c', state%v_c(:,:), 'restart')
