...
contains
  ...
  subroutine history_write_state(state, static, diag)
    ...
    do j = parallel%full_lat_start_idx, parallel%full_lat_end_idx
      do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
        u(i,j) = 0.5 * (state%u(i,j,combined_wind_idx) + state%u(i-1,j,combined_wind_idx))
      end do
    end do
    do j = parallel%full_lat_start_idx_no_pole, parallel%full_lat_end_idx_no_pole
      do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
        v(i,j) = 0.5 * (state%v(i,j,combined_wind_idx) + state%v(i,j-1,combined_wind_idx))
      end do
    end do
  ...
  subroutine history_write_tendency(state, tend, tag)
    ...
    call io_output('u_adv_lon', tend%u_adv_lon(:,:,1), 'debug')
    call io_output('u_adv_lat', tend%u_adv_lat(:,:,1), 'debug')
    call io_output('v_adv_lon', tend%v_adv_lon(:,:,1), 'debug')
    call io_output('v_adv_lat', tend%v_adv_lat(:,:,1), 'debug')
    call io_output('fv', tend%fv(:,:,1), 'debug')
    call io_output('fu', tend%fu(:,:,1), 'debug')
    ...
    call io_output('du', tend%du(:,:,1), 'debug')
    call io_output('dv', tend%dv(:,:,1), 'debug')
    call io_output('dgd', tend%dgd(:,:), 'debug')
    call io_output('iap_u', state%iap%u(:,:,combined_wind_idx), 'debug')
    call io_output('iap_v', state%iap%v(:,:,combined_wind_idx), 'debug')
