...
contains
  ...
  subroutine diag_init()
    ...
    if (.not. allocated(diag%vor)) call parallel_allocate(diag%vor, half_lat=.true., extended_halo=.true.)
    if (.not. allocated(diag%div)) call parallel_allocate(diag%div, half_lat=.true., extended_halo=.true.)
